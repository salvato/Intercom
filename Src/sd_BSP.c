...
//          it is ready for access. The access can be performed either using the polling
...
BSP_SD_Init(void) {
    ...
    // Check if SD card is present (on Pin D3)
...
BSP_SD_MspInit(SD_HandleTypeDef *hsd, void *Params) {
    ...
    // GPIOC configuration       PC8          PC9          PC10          PC11          PC12
    // SDIO -------------->       D0           D1           D2            D3            CK
    gpio_init_structure.Pin = GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 | GPIO_PIN_12;

    HAL_GPIO_Init(GPIOC, &gpio_init_structure);

    // GPIOD configuration      PD2
    // SDIO -------------->     Cmd I/O
...
BSP_SD_Detect_MspInit(SD_HandleTypeDef *hsd, void *Params) {
    ...
    // GPIO configuration in input for uSD_Detect signal (PD3)
    gpio_init_structure.Pin       = SD_DETECT_PIN;
...
BSP_SD_MspDeInit(SD_HandleTypeDef *hsd, void *Params) {
    ...
    // (by overloading this __weak function)
